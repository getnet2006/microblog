...
@bp.route('/<username>/list_followers')
@login_required
def list_followers(username):
    page = request.args.get('page', 1, type=int)
    users = User.query.filter_by(username=username).first().followers.paginate(page, current_app.config['POSTS_PER_PAGE'], False)
...
def list_following(username):
    page = request.args.get('page', 1, type=int)
    users = User.query.filter_by(username=username).first().followed.paginate(page, current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.<username>/list_following', username=user.username, page=users.next_num) if users.has_next else None
    prev_url = url_for('main.<username>/list_following', username=user.username, page=users.prev_num) if users.has_prev else None
    form = EmptyForm()
...
def explore():
    page = request.args.get('page', 1, type=int)
    posts = Post.query.order_by(Post.timestamp.desc()).filter(Post.user_id != current_user.id).paginate(
